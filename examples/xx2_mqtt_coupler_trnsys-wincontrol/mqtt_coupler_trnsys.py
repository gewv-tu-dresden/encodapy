...
class MQTTCouplerTrnsys(ControllerBasicService):
    """
    Class for a coupler between testrig and TRNSYS
    ...
    def prepare_start(self) -> None:
        ...
        logger.info("TRNSYS Coupler Service prepared successfully")
    ...
    async def calculation(self, data: InputDataModel) -> Union[DataTransferModel, None]:
        ...
        wincontrol_input_entity = self.get_input_entity(data, "WinControl-Outputs")

        # check if the input entities are updated after the last output
        trnsys_updated = self.entity_fully_updated_after_last_output(
            trnsys_input_entity
        )
        wincontrol_updated = self.entity_fully_updated_after_last_output(
            wincontrol_input_entity
        )
        ...
        # if wincontrol_updated is false, return with no outputs to wait for new inputs
        if not wincontrol_updated:
            return None

        # get the inputs needed by the controller from the input entities
        input_entities = [trnsys_input_entity, wincontrol_input_entity]
        ...
        self.service_outputs["t_heat"] = self.service_inputs["t_heat_actual"]
        ...
        # add the full_trnsys_message to the components
        components.append(
            DataTransferComponentModel(
