...
An overview is available as Pydantic BaseModel {py:class}`~encodapy.components.thermal_storage.thermal_storage_config.ThermalStorageOutputData` in [thermal_storage_config.py](https://github.com/gewv-tu-dresden/encodapy/blob/v0.10.1/encodapy/components/thermal_storage/thermal_storage_config.py).

If the Temperature Check (`load_level_check` in {py:class}`~encodapy.components.thermal_storage.thermal_storage_config.ThermalStorageConfigData`) is enabled, the temperature of the sensor is checked and if the levels fall below the limits, the charge level is adjusted. See <a href="#component-configuration">Component Configuration</a> for the information on adding this to temperature sensors.
...
Configuration parameters must be set as datapoints or connections to static data in the config file. For more details, see the {py:class}`~encodapy.components.thermal_storage.thermal_storage_config.ThermalStorageConfigData` in [thermal_storage_config.py](https://github.com/gewv-tu-dresden/encodapy/blob/v0.10.1/encodapy/components/thermal_storage/thermal_storage_config.py) or the documentation.
...
For detailed documentation of the inputs, see the {py:class}`~encodapy.components.thermal_storage.thermal_storage_config.ThermalStorageInputData` in [thermal_storage_config.py](https://github.com/gewv-tu-dresden/encodapy/blob/v0.10.1/encodapy/components/thermal_storage/thermal_storage_config.py) or the documentation.

## Example

An example how the component could be used is available in [examples/06_thermal_storage_service](https://github.com/gewv-tu-dresden/encodapy/blob/v0.10.1/examples/06_thermal_storage_service)