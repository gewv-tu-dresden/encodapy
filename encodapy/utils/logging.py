...
def _normalize_numeric_value(
    value: int | str | time | timedelta | None,
) -> int | str | time | timedelta | None:
...
class LoggerControl:
    ...
    def prepare_file_logger(
        ...
        retention: Optional[int | str | timedelta],
    ) -> None:
        """
        ...
        rotation = _normalize_numeric_value(rotation)
        retention = cast(
            Optional[int | str | timedelta], _normalize_numeric_value(retention)
        )
        logger.add(
            path,
            level=log_level,
