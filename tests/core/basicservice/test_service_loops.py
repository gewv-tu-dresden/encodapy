...
class TestHoldSamplingTime:
    ...
    @pytest.mark.asyncio
    async def test_hold_sampling_time_already_exceeded(
        self, capsys, unset_shutdown_event
    ):
        """
...
class TestServiceLoops:
    ...
    async def test_start_service(self, basic_service):
        ...
        with (
            patch.object(basic_service, "get_data", side_effect=count_get_data),
            patch.object(basic_service, "calculation", side_effect=count_calculation),
            patch.object(
                basic_service, "prepare_output", side_effect=count_prepare_output
            ),
            ...
            patch.object(
                basic_service,
                "_hold_sampling_time",
                side_effect=mock_hold_sampling_time,
            ),
        ):
    ...
    async def test_start_calibration(self, basic_service):
        ...
        basic_service.config.controller_settings.time_settings.calibration = (
    ...
    async def test_check_health_status(self, basic_service, unset_shutdown_event):
        ...
        with (
            patch(
                "encodapy.service.basic_service.update_health_file",
        ...
        assert (
            call_counts["update_health"] >= 1
        ), f"update_health called {call_counts['update_health']} times"
        assert (
            call_counts["hold_sampling"] >= 1
        ), f"hold_sampling called {call_counts['hold_sampling']} times"
