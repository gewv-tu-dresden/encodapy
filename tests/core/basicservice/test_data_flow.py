...
class TestGetData:
    ...
    async def test_get_data(self, basic_service):
        ...
        with (
            patch.object(basic_service, "get_data_from_fiware") as mock_get_data_fiware,
            ...
            patch.object(
                basic_service, "_get_last_timestamp_for_fiware_output"
            ) as mock_get_last_timestamp_fiware,
            patch.object(
                basic_service, "_get_last_timestamp_for_file_output"
            ) as mock_get_last_timestamp_file,
            patch.object(
                basic_service, "_get_last_timestamp_for_mqtt_output"
            ) as mock_get_last_timestamp_mqtt,
        ):
            ...
            mock_get_data_file.return_value = InputDataEntityModel(
                id="input_2", attributes=[]
            )
            mock_get_data_mqtt.return_value = InputDataEntityModel(
                id="input_3", attributes=[]
            )

            # Return OutputDataEntityModel objects for timestamps (not None)
            mock_get_last_timestamp_fiware.return_value = (
                OutputDataEntityModel(id="output_1", attributes=[], commands=[]),
                None,
            )
            mock_get_last_timestamp_file.return_value = (
                OutputDataEntityModel(id="output_2", attributes=[], commands=[]),
                None,
            )
            mock_get_last_timestamp_mqtt.return_value = (
                OutputDataEntityModel(id="output_3", attributes=[], commands=[]),
                None,
            )
    ...
    async def test_get_data_no_config(self, service_with_no_config):
        ...
        result = await service_with_no_config.get_data(
    ...
    async def test_get_data_skips_missing_fiware_and_file_data(self, basic_service):
        ...
        with (
            ...
            patch.object(
                basic_service,
                "_get_last_timestamp_for_file_output",
                return_value=(
                    OutputDataEntityModel(id="output_2", attributes=[], commands=[]),
                    None,
                ),
            ...
            patch.object(
                basic_service,
                "_get_last_timestamp_for_mqtt_output",
                return_value=(
                    OutputDataEntityModel(id="output_3", attributes=[], commands=[]),
                    None,
                ),
...
class TestSendOutputs:
    ...
    async def test_send_outputs(self, basic_service):
        ...
        output_entity = OutputModel(
            ...
            attributes=[
                AttributeModel(
                    id="test_attr", type=AttributeTypes.VALUE, datatype=DataType.TEXT
                ),
            ],
            commands=[
        ...
        with (
            patch.object(
                basic_service, "_get_output_entity_config"
            ) as mock_get_entity_config,
            patch.object(
                basic_service, "_get_output_attribute_config"
            ) as mock_get_attr_config,
            patch.object(
                basic_service, "_get_output_command_config"
            ) as mock_get_command_config,
    ...
    async def test_send_outputs_no_matching_entity_config(self, basic_service):
        ...
        output_data = OutputDataModel(
            entities=[
                OutputDataEntityModel(
                    id="nonexistent_entity", attributes=[], commands=[]
                )
            ]
        )
        basic_service.config = MagicMock()
    ...
    async def test_send_outputs_no_attributes_or_commands(self, basic_service):
        ...
        output_data = OutputDataModel(
            entities=[
                OutputDataEntityModel(id="test_entity", attributes=[], commands=[])
            ]
        )
    ...
    def test_prepare_output_multiple_components_same_entity(self, basic_service):
        ...
        basic_service.config.outputs = [
            OutputModel(
                ...
                attributes=[
                    AttributeModel(
                        id="attr_1", type=AttributeTypes.VALUE, datatype=DataType.NUMBER
                    ),
                    AttributeModel(
                        id="attr_2", type=AttributeTypes.VALUE, datatype=DataType.NUMBER
                    ),
                ],
                commands=[],
            )
...
class TestGetOutputAttributeConfig:
    ...
    def test_get_output_attribute_config_found(self, service_with_no_config):
        ...
        entity = OutputModel(
            ...
            attributes=[
                AttributeModel(
                    id="temp", type=AttributeTypes.VALUE, datatype=DataType.NUMBER
                ),
                AttributeModel(
                    id="humidity", type=AttributeTypes.VALUE, datatype=DataType.NUMBER
                ),
            ],
            commands=[],
        )
    ...
    def test_get_output_attribute_config_not_found_entity(self, service_with_no_config):
        ...
        result = service_with_no_config._get_output_attribute_config(
            "nonexistent_entity", "temp"
        )

        assert result is None

    def test_get_output_attribute_config_not_found_attribute(
        self, service_with_no_config
    ):
        """
...
class TestGetOutputCommandConfig:
    ...
    def test_get_output_command_config_not_found_entity(self, service_with_no_config):
        ...
        result = service_with_no_config._get_output_command_config(
            "nonexistent_entity", "reset"
        )
